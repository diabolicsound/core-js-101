...
function getDigitalRoot(num) {
  const reduced = String(num).split('').reduce((sum, current) => sum + +current, 0);
  return +reduced > 9 ? getDigitalRoot(+reduced) : +reduced;
}
...
function getCommonDirectoryPath(pathes) {
  ...
  const empty = powerArr;
  const ret = powerArr.join('/');
  let str = ret;
  str = ret.slice(1);
  if (empty.length === 0) {
    return '';
...
function getMatrixProduct(/* m1, m2 */) {
  ...
  const vertOne = m1[0].length;
  const vert = m2[0].length;
  const arr = Array(rows).fill(Array(vert).fill());
  console.log(arr, m2);
